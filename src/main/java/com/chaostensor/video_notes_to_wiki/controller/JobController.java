...
public class JobController {
    ...
    public JobController(JobRepository jobRepository, VideoProcessingService videoProcessingService, ObjectMapper objectMapper) {
    ...
    @PostMapping
    public Mono<ResponseEntity<JobResponse>> createJob(@RequestBody JobRequest request) {
        Job job = new Job();
        job.setId(UUID.randomUUID());
        job.setStatus(JobStatus.PENDING);
        job.setInputDir(request.getInputDir() != null ? request.getInputDir() : "/default/videos");

        return jobRepository.save(job)
            .doOnNext(savedJob -> {
                // Start async processing
                videoProcessingService.processJob(savedJob).subscribe();
            })
            .map(savedJob -> ResponseEntity.ok(new JobResponse(savedJob.getId())));
    }
